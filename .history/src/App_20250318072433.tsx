          ...
          <div className="flex gap-4 mb-6">
            <div className="w-1/2">
              ...
              <input
                ...
                onBlur={() => {
                  const match = currencies?.find(({ code, name }) =>
                    code.toLowerCase().includes(fromSearch.toLowerCase()) ||
                    name.toLowerCase().includes(fromSearch.toLowerCase())
                  );
                  if (match) dispatch(setFromCurrency(match.code));
                }}
            ...
            <div className="w-1/2">
              ...
              <input
                ...
                onBlur={() => {
                  const match = currencies?.find(({ code, name }) =>
                    code.toLowerCase().includes(toSearch.toLowerCase()) ||
                    name.toLowerCase().includes(toSearch.toLowerCase())
                  );
                  if (match) dispatch(setToCurrency(match.code));
                }}
                className="w-full mt-1 pl-4 pr-4 py-3 border rounded-lg text-lg focus:outline-none focus:ring-2 focus:ring-blue-400"
                placeholder="Search currency..."
              />
