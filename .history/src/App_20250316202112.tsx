...
import { ResponsiveContainer, XAxis, YAxis, Tooltip, CandlestickChart, Candlestick } from "recharts";
...
function App() {
  ...
  const fetchExchangeRates = async () => {
    try {
      ...
      // Mock data structure (normally fetched from a historical API)
      const data = Object.keys(rates).map((currency, index) => ({
        currency,
        open: rates[currency] * (1 - Math.random() * 0.02), // Simulating open price
        high: rates[currency] * (1 + Math.random() * 0.02), // Simulating high price
        low: rates[currency] * (1 - Math.random() * 0.03), // Simulating low price
        close: rates[currency], // Actual exchange rate
      }));
  ...
  const handleConvert = () => {
    const rate = exchangeRates.find(rate => rate.currency === toCurrency)?.close || 1;
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white p-8">
      <div className="flex flex-row gap-8 w-full max-w-[1400px]">
        ...
        {/* Exchange Rate Candlestick Chart */}
        <div className="w-1/2 p-6 bg-white rounded-xl ">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Exchange Rate Trends</h3>
          <ResponsiveContainer width="100%" height={400}>
            <CandlestickChart data={exchangeRates}>
              <XAxis dataKey="currency" />
              <YAxis />
              <Tooltip />
              <Candlestick dataKey="open" highKey="high" lowKey="low" closeKey="close" stroke="#2563eb" fill="#2563eb" />
            </CandlestickChart>
          </ResponsiveContainer>
        </div>
      </div>
