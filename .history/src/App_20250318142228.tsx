...
function CurrencyConverter() {
  ...
  <div className="mb-6">
    <label className="block text-gray-700 text-base font-medium">Enter Amount</label>
    <div className="relative">
      <input
        type="number"
        value={amount}
        onChange={(e) => dispatch(setAmount(e.target.value))}
        className="w-full mt-1 pl-12 pr-4 py-3 border border-gray-300 rounded-lg text-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
  ...
  <div className="flex gap-4 mb-6">
    {/* From Currency */}
    <div className="w-1/2">
      <label className="block text-gray-700 text-base font-medium">From</label>
      <div className="relative flex items-center">
        <img src={getFlagUrl(fromCurrency)} alt={fromCurrency} className="w-6 h-6 absolute left-2 rounded-full" />
        <select
          value={fromCurrency}
          onChange={(e) => dispatch(setFromCurrency(e.target.value))}
          className="w-full mt-1 pl-10 pr-4 py-3 border border-gray-300 rounded-lg text-lg focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none bg-white"
        >
          {isLoading ? (
    ...
    {/* To Currency */}
    <div className="w-1/2">
      <label className="block text-gray-700 text-base font-medium">To</label>
      <div className="relative flex items-center">
        <img src={getFlagUrl(toCurrency)} alt={toCurrency} className="w-6 h-6 absolute left-2 rounded-full" />
        <select
          value={toCurrency}
