...
function CurrencyConverter() {
          ...
          <div className="mb-6">
            ...
            <div className="relative">
              <input
                ...
                className="w-full mt-1 pl-12 pr-4 py-3 border-4 border-gray-500 rounded-lg text-lg focus:outline-none focus:ring-2 focus:ring-blue-400"
  ...
  <div className="relative flex items-center">
    <img src={getFlagUrl(fromCurrency)} alt={fromCurrency} className="w-6 h-6 absolute left-2" />
    <select
      value={fromCurrency}
      onChange={(e) => dispatch(setFromCurrency(e.target.value))}
      className="w-full mt-1 pl-10 pr-4 py-3 border rounded-lg text-lg focus:outline-none focus:ring-2 focus:ring-blue-400 appearance-none bg-white"
    >
      {isLoading ? (
        <option>Loading...</option>
      ) : (
        currencies?.map(({ code, name }: Currency) => (
          <option key={code} value={code} style={{ backgroundImage: `url(${getFlagUrl(code)})`, backgroundSize: '20px', paddingLeft: '30px' }}>
            {code} - {name}
          </option>
        ))
      )}
...
<div className="w-1/2">
  ...
  <div className="relative flex items-center">
    <img src={getFlagUrl(toCurrency)} alt={toCurrency} className="w-6 h-6 absolute left-2" />
    <select
      value={toCurrency}
