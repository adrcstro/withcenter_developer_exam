...
function CurrencyConverter() {
  ...
  return (
    ...
    <div className="flex flex-col justify-center w-full items-center max-w-[2000px] md:px-8 px-4">
      <div className="bg-white p-6 rounded-xl w-full max-w-3xl md:p-8">
      <h2 className="flex justify-center items-center text-center text-gray-800 font-semibold font-serif gap-2 mb-4 sm:mb-5 md:mb-6 lg:mb-8 text-2xl sm:text-3xl md:text-4xl lg:text-5xl">
  <CurrencyDollarIcon className="h-6 w-6 sm:h-8 sm:w-8 md:h-10 md:w-10 lg:h-12 lg:w-12 text-yellow-500" />
  Currency Converter
</h2>

  
        <div className="w-full mb-4 md:mb-6">
          <label className="text-base text-gray-600 block font-medium">Enter Amount</label>
          <div className="relative">
            <input
              type="number"
              value={amount}
              onChange={(e) => dispatch(setAmount(e.target.value))}
              className="border border-gray-300 rounded-xl shadow-sm text-base w-full duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 md:pl-12 md:py-3 md:text-lg mt-1 pl-10 pr-4 py-2 transition-all"
              placeholder="Enter amount"
            />
            <CurrencyDollarIcon className="h-6 text-gray-500 w-6 -translate-y-1/2 absolute left-3 md:h-7 md:w-7 top-1/2" />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4 mb-4 md:flex-row md:mb-6">
          ...
          <div className="w-full md:w-1/2">
            <label className="text-base text-gray-600 block font-medium">From</label>
            <div className="flex items-center relative">
              <img
                src={getFlagUrl(fromCurrency)}
                alt={fromCurrency}
                className="border border-gray-300 h-5 rounded-full w-5 absolute left-3 md:h-6 md:w-6"
              />
              <select
                value={fromCurrency}
                onChange={(e) => dispatch(setFromCurrency(e.target.value))}
                className="bg-white border border-gray-300 rounded-xl shadow-sm text-base w-full appearance-none duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 md:pl-12 md:pr-10 md:py-3 md:text-lg mt-1 pl-10 pr-8 py-2 transition-all"
              >
                {isLoading ? (
              ...
              <ChevronDownIcon className="h-4 text-gray-600 w-4 absolute md:h-5 md:w-5 pointer-events-none right-3" />
            </div>
          </div>
  
          {/* To Currency */}
          <div className="w-full md:w-1/2">
            <label className="text-base text-gray-600 block font-medium">To</label>
            <div className="flex items-center relative">
              <img
        ...
        <button
          ...
          className="flex bg-blue-600 justify-center rounded-lg text-base text-white w-full duration-300 font-semibold gap-2 hover:bg-blue-700 items-center md:py-3 md:text-lg py-2 transition"
        >
        ...
        {convertedAmount && (
          <div className="text-center md:mt-6 mt-4">
